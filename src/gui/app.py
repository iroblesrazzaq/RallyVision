...
class JsApi:
    """JavaScript API exposed to pywebview for native file operations."""
    ...
    def download_video(self, job_id: str) -> dict:
        """Download video using native file save dialog."""
        import webview
        ...
        if job is None:
            return {"success": False, "error": "Unknown job id"}
        ...
        suggested_name = Path(video_path).name
        ...
        if self._window:
            ...
            if save_path:
                # save_path is a tuple for SAVE_DIALOG, get the string
                dest = save_path if isinstance(save_path, str) else save_path[0] if save_path else None
                if dest:
                    try:
    ...
    def download_csv(self, job_id: str) -> dict:
        """Download CSV using native file save dialog."""
        import webview
        ...
        if not csv_path or not os.path.exists(csv_path):
            return {"success": False, "error": "CSV not available"}
...
def launch(port: Optional[int] = None) -> int:
    ...
    # Create API instance for native file operations
    js_api = JsApi()
    ...
    # Set window reference so API can use file dialogs
