#!/usr/bin/env python3
"""
check_all.py - Create check annotations for all videos with color-coded bounding boxes

This script finds all video files in raw_videos and creates annotated videos in the 'check' directory
with color-coded bounding boxes:
- Green boxes: centroids inside the playable court area
- Blue boxes: centroids outside the playable court area
- Red overlay: court mask "out" areas

Usage:
    python check_all.py [pose_data_dir] [sample_duration] [overwrite]
    
    pose_data_dir: Path to the pose data directory (e.g., "pose_data/unfiltered/yolos_0.2conf_10fps_0s_to_999999s")
...
Examples:
    python check_all.py "pose_data/unfiltered/yolos_0.2conf_10fps_0s_to_999999s"                    # 10-minute samples, no overwrite
    python check_all.py "pose_data/unfiltered/yolos_0.2conf_10fps_0s_to_999999s" 15                 # 15-minute samples, no overwrite
    python check_all.py "pose_data/unfiltered/yolos_0.2conf_10fps_0s_to_999999s" 10 true            # 10-minute samples, overwrite existing
"""
...
def run_check_command(video_path, pose_data_path, segment_start, segment_duration, overwrite=False):
    """
    Run the check annotation command.
...
def main():
    ...
    if len(sys.argv) >= 2:
        ...
        print("Usage: python check_all.py [pose_data_dir] [segment_duration] [overwrite]")
        return False
    ...
    print("🎯 Creating Check Annotations for All Videos")
    print("=" * 60)
    ...
    print(f"Features: Color-coded bounding boxes (Green=in-court, Blue=out-of-court), court mask overlay")
    ...
    for i, video_path in enumerate(video_files, 1):
        ...
        # Take a single sample from the beginning of the video
        ...
        base_name = os.path.splitext(os.path.basename(video_path))[0]
        output_filename = f"{base_name}_annotated_{segment_start}s_to_{segment_start + actual_segment_duration}s_yolos.mp4"
        output_path = f"check/yolos_0.2conf_15fps_{segment_start}s_to_{segment_start + actual_segment_duration}s/{output_filename}"
        
        if os.path.exists(output_path) and not overwrite:
            print(f"⏭️  Skipping - check video already exists: {output_filename}")
            continue
        
        # Run check annotation
        if run_check_command(video_path, pose_data_path, segment_start, actual_segment_duration, overwrite):
            successful += 1
        else:
            failed += 1
    ...
    print(f"\n🎉 CHECK ANNOTATIONS COMPLETED")
    print("=" * 60)
    print(f"Total videos processed: {len(video_files)}")
    print(f"Successful check videos: {successful}")
    print(f"Failed check videos: {failed}")
    print(f"Total runtime: {total_time:.2f} seconds ({total_time/60:.2f} minutes)")
    
    if successful > 0:
        print(f"Average time per check video: {total_time/successful:.2f} seconds")
